...
function unitDps(key) {
  const u = SOLDIER_TYPES[key];
  return u ? +(u.dmg / u.attackRate).toFixed(2) : 0;
}
function unitEhp(key) {
  const u = SOLDIER_TYPES[key];
  return u ? u.hp : 0;
}
...
// Total HP and DPS of a wave (waveIdx) for budgeting
function waveStats(waveIdx) {
  const types = Object.values(ENEMY_TYPES);
  const count = WAVE_BASE + waveIdx * WAVE_GROWTH;
  const avgHp  = types.reduce((s,t) => s + t.hpBase + waveIdx * 10, 0) / types.length;
  const avgDps = types.reduce((s,t) => s + (t.dmg + waveIdx * 2) / t.attackRate, 0) / types.length;
  return { count, totalHp: +(count * avgHp).toFixed(0), totalDps: +(count * avgDps).toFixed(1) };
}

// ── Fortification definitions ─────────────────────────
const FORT_DEFS = {
  wall:      { name: 'Coral Wall',   cost: { wood: 20, iron: 0  }, hp: 350, range: 0,   dmg: 0,  fireRate: 0   },
  tower:     { name: 'Torpedo Post', cost: { wood: 30, iron: 20 }, hp: 150, range: 408, dmg: 30, fireRate: 1.8 },
...
// Maps building type → array of { type, level } requirements.
// Buildings not listed here have no prerequisites.
const BLDG_DEPS = {
  coal_mine:    [{ type: 'furnace',    level: 1 }],
  iron_mine:    [{ type: 'coal_mine',  level: 1 }],
  barracks:     [{ type: 'chief_hall', level: 2 }],
  hospital:     [{ type: 'barracks',   level: 1 }],
  warehouse:    [{ type: 'chief_hall', level: 2 }],
  research_lab: [{ type: 'barracks',   level: 2 }],
  city_wall:    [{ type: 'chief_hall', level: 3 }],
};

// ── Unit prerequisite tree ────────────────────────────
// Maps unit key → { bldgType: minLevel, ... }.
// Tier-0 base units need barracks lv1; tiers 1/2 also need the research lab.
const UNIT_DEPS = {
  // Mammal
  dolphin:    { barracks: 1 },
  orca:       { barracks: 1 },
  seal:       { barracks: 1 },
  dolphin_t1: { barracks: 2, research_lab: 1 },
  orca_t1:    { barracks: 2, research_lab: 1 },
  seal_t1:    { barracks: 2, research_lab: 1 },
  dolphin_t2: { barracks: 3, research_lab: 2 },
  orca_t2:    { barracks: 3, research_lab: 2 },
  seal_t2:    { barracks: 3, research_lab: 2 },
  // Human
  diver_t0:   { barracks: 1 },
  gunner_t0:  { barracks: 1 },
  sub_t0:     { barracks: 2 },
  diver_t1:   { barracks: 2, research_lab: 1 },
  gunner_t1:  { barracks: 2, research_lab: 1 },
  sub_t1:     { barracks: 2, research_lab: 1 },
  diver_t2:   { barracks: 3, research_lab: 2 },
  gunner_t2:  { barracks: 3, research_lab: 2 },
  sub_t2:     { barracks: 3, research_lab: 2 },
  // Fish
  ray:        { barracks: 1 },
  puffer_t0:  { barracks: 1 },
  angler_t0:  { barracks: 1 },
  ray_t1:     { barracks: 2, research_lab: 1 },
  puffer_t1:  { barracks: 2, research_lab: 1 },
  angler_t1:  { barracks: 2, research_lab: 1 },
  ray_t2:     { barracks: 3, research_lab: 2 },
  puffer_t2:  { barracks: 3, research_lab: 2 },
  angler_t2:  { barracks: 3, research_lab: 2 },
};

// Returns { ok: true } or { ok: false, missing: 'Human-readable requirement' }
function canBuildDeps(btype, buildingList) {
  const reqs = BLDG_DEPS[btype];
  if (!reqs) return { ok: true };
  for (const { type: rt, level: rl } of reqs) {
    const found = buildingList.find(b => b.type === rt && b.level >= rl && !b.constructing);
    if (!found) {
      const name = BLDG_DEFS[rt]?.name ?? rt;
      return { ok: false, missing: `Needs ${name} Lv${rl}` };
    }
  }
  return { ok: true };
}
...
// Returns { ok: true } or { ok: false, missing: 'Human-readable requirement' }
function canResearchDeps(unitKey, buildingList) {
  const reqs = UNIT_DEPS[unitKey];
  if (!reqs) return { ok: true };
  for (const [btype, minLevel] of Object.entries(reqs)) {
    const found = buildingList.find(b => b.type === btype && b.level >= minLevel && !b.constructing);
    if (!found) {
      const name = BLDG_DEFS[btype]?.name ?? btype;
      return { ok: false, missing: `Needs ${name} Lv${minLevel}` };
    }
  }
  return { ok: true };
}
